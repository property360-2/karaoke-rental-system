...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name || !$email || !$password || !$confirm) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            $stmt = $conn->prepare('INSERT INTO users (name, email, password, phone, address, role) VALUES (?, ?, ?, ?, ?, "admin")');
            $stmt->bind_param('sssss', $name, $email, $hash, $phone, $address);
            if ($stmt->execute()) {
                $success = 'Admin registration successful! <a href="login.php">Login here</a>.';
            } else {
    ...
    <div class="row justify-content-center">
        <div class="col-md-6">
            <div class="card theme-navbar p-4">
                <h2 class="mb-4 theme-title text-center">Admin Registration</h2>
                <?php if ($errors): ?>
                ...
                <form method="post" novalidate>
                    ...
                    <div class="mt-3 text-center">
                        <a href="login.php" class="theme-text">Back to Login</a>
                    </div>
                </form>
            </div>
        </div>
